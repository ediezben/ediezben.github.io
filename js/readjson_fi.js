...
function constructTable(selector) {
      ...
      function Headers(list, selector) {
          var columns = [];
          var html_head = '<thead style="color:black;"><tr><th>Prosessin tyyppi:</th></tr><thead>';
          ...
          for (var i = 0; i < keys.length; i++) {
            ...
            html_t+='<tr style="color:black;background-color:rgba(0,106,137,0.5);text-align:center;cursor:pointer" data-toggle="collapse" data-target="#' + key_id +'" class="accordion-toggle"><td>'+first_key+' <a onclick="event.stopPropagation();" href="descriptions_fi.html#'+id_general_1+'" target="_blank"><i class="glyphicon glyphicon-info-sign" style="cursor:pointer"></i></a></td></tr>';
            ...
            for (var k = 0; k < keys_step_1.length; k++) {
              ...
              html_t+='<tr style="color:black;background-color:rgba(54,125,20,0.5);text-align:center;cursor:pointer" data-toggle="collapse" data-target="#' + second_key_id +'" class="accordion-toggle id="'+key_id+'"><td>'+second_key+' <a onclick="event.stopPropagation();" href="descriptions_fi.html#'+id_general_2+'" target="_blank"><i class="glyphicon glyphicon-info-sign" style="cursor:pointer"></i></a></td></tr>';
              ...
              html_t+='<table class="table table-striped">';
              var html_head_2 = '<thead style="color:black;"><tr><th>Valitse työvaihe:</th></tr><thead>';
              html_t += html_head_2;
              for (var m = 0; m < keys_step_2.length; m++) {
                ...
                html_t+='<tr style="color:black;background-color:rgba(136,176,54,0.5);text-align:center;cursor:pointer" data-toggle="collapse" data-target="#' + third_key_id +'" class="accordion-toggle id="'+second_key_id+'"><td>'+third_key+' <a onclick="event.stopPropagation();" href="descriptions_fi.html#'+id_general_3+'" target="_blank"><i class="glyphicon glyphicon-info-sign" style="cursor:pointer"></i></a></td></tr>';
                html_t+='<tr><td colspan="12" class="hiddenRow"><div class="accordian-body collapse" id="'+third_key_id+'">';
                html_t+='<table class="table table-striped">';
                html_t+='<thead><tr style="color:black;background-color:rgba(179,198,73,0.5)"><th>Tehtävä</th><th>Tekoälytyökalun kuvaus</th><th>Tulosten kuvaus</th><th>Tehokkuus ihanneoloissa KPI (keskeinen tulosindikaattori)</th><th>Tehokkuus resurssien käytössä KPI (keskeinen tulosindikaattori)</th><th>Toiminta</></thead>';
                // alert(obj_step_3)
                for (var n = 0; n < obj_step_3.length; n++) {
                    ...
                    for (var colIndex = 0; colIndex < cols.length; colIndex++)
                    {
                        ...
                        if(cols[colIndex]=="Tasks"){
                          html_t += '<td id="'+val_code+'">'+val+' <a href="descriptions_fi.html#'+val_code_tsk+'" target="_blank"><i class="glyphicon glyphicon-info-sign" style="cursor:pointer"></i></a></td>';
                          ...
                          html_t += '<td>'+val+' <a href="descriptions_fi.html#'+val_code+'" target="_blank"><i class="glyphicon glyphicon-info-sign" style="cursor:pointer"></i></a></td>';
                          // html_t+='<td><input type="number" id="quantity" name="quantity" min="0" max="999999999" value="'+val+'"><td>';
                        }else{
                          html_t += '<td>'+val+'</td>';
                        }
                    ...
                    html_t +='<td>'+'<div><button class="btn" style="width:100%;background-color:#CFF4D2" onclick=generatePDF(\''+third_key_id+'\',\''+ name_row +'\')><i class="fa fa-download"></i> Lataa</button></div>'+'</td>'
                    html_t +='</tr>';
                    // Adding each row to the table
                    // $(selector).append(row);
                }
